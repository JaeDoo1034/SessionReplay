(() => {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #${PANEL_ID} input[type='file']::file-selector-button {
        border: 0;
        border-radius: 8px;
        padding: 6px 10px;
        margin-right: 8px;
      ...
      #${PANEL_ID} input[type='file']::-webkit-file-upload-button {
        border: 0;
        border-radius: 8px;
        padding: 6px 10px;
        margin-right: 8px;
  ...
  function applyMutation(doc, data) {
    ...
    if (data.mutationType === "childList") {
      ...
      const patched = applyChildListMutationPatch(doc, target, data);
      if (patched) {
        return;
      }
  ...
  function applyChildListMutationPatch(doc, target, data) {
    if (!(target instanceof Element)) {
      return false;
    }
    ...
    addedNodes.forEach((nodeDesc) => {
      if (appendSerializedNode(target, nodeDesc)) {
        changed = true;
      }
    });
  ...
  function removeSerializedNode(doc, target, nodeDesc) {
    ...
    if (nodeDesc.nodeType === "text") {
      ...
      const textNode = Array.from(target.childNodes).find((node) => {
        return node.nodeType === Node.TEXT_NODE && node.textContent === text;
      });
      if (textNode) {
        textNode.remove();
        return true;
  ...
  function appendSerializedNode(target, nodeDesc) {
    if (!nodeDesc || !(target instanceof Element)) {
      return false;
    }
    ...
    if (nodeDesc.nodeType === "element" && typeof nodeDesc.outerHTML === "string") {
      ...
      template.innerHTML = sanitizeFragmentHtml(nodeDesc.outerHTML);
