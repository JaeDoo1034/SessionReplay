(() => {
  ...
  function applyMutation(doc, data, options = {}) {
    ...
    if (data.mutationType === "childList") {
      ...
      const canFallbackToInnerHtml = typeof data.targetInnerHTML === "string" && data.targetInnerHTML !== "[redacted]";

      if (patchResult.complete && patchResult.changed) {
        return;
      }

      if (canFallbackToInnerHtml) {
  ...
  function appendSerializedNode(target, nodeDesc, options = {}) {
    ...
    if (nodeDesc.nodeType === "text") {
      target.appendChild(ownerDoc.createTextNode(String(nodeDesc.textContent || "")));
      return true;
    }
