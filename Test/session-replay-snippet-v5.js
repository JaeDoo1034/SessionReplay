(() => {
  ...
  function bootstrap() {
    ...
    window.SessionReplaySnippet = {
      ...
      loadPayload,
      playReplay,
      setSnapshotIntervalMs: (ms) => recorder.setSnapshotIntervalMs(ms),
      captureSnapshotNow: (reason = "manual_capture") => recorder.captureSnapshot(reason, true),
      setReplayMutationMode: (enabled) => replayer.setApplyMutationEvents(Boolean(enabled)),
      setReplayScriptMode: (enabled) => replayer.setExecutePageScripts(Boolean(enabled)),
    ...
    setStatus(`Snippet loaded. Start to record. snapshotIntervalMs=${recorder.snapshotIntervalMs}`);
  ...
  function bindUI() {
    ...
    ui.snapshotIntervalInput.value = String(recorder.snapshotIntervalMs);
    ...
    ui.toggleScriptBtn.addEventListener("click", () => {
      const enabled = replayer.setExecutePageScripts(!replayer.executePageScripts);
      ui.toggleScriptBtn.textContent = enabled ? "Scripts ON" : "Scripts OFF";
      setReplayStatus(`Replay mode updated (${getReplayModeText()}).`);
    });

    ui.snapshotIntervalInput.addEventListener("change", () => {
      const ms = Number(ui.snapshotIntervalInput.value);
      const applied = recorder.setSnapshotIntervalMs(ms);
      ui.snapshotIntervalInput.value = String(applied);
      setStatus(`Snapshot interval updated: ${applied}ms`);
    });
  ...
  function start() {
    ...
    setStatus(`Recording started. periodic snapshot=${recorder.snapshotIntervalMs}ms`);
  }
  ...
  function stop() {
    ...
    const snapshotCount = lastPayload.events.filter((event) => event.type === "snapshot").length;
    setStatus(`Recording stopped. eventCount=${lastPayload.eventCount}, snapshots=${snapshotCount}`);
  }
  ...
  function help() {
    return {
      ...
      setSnapshotIntervalMs: "Update periodic snapshot interval in ms (min 500)",
      captureSnapshotNow: "Force-capture a snapshot immediately",
      setReplayMutationMode: "Enable/disable mutation event application during replay",
      setReplayScriptMode: "Enable/disable script execution in replay iframe (default OFF)",
  ...
  function getReplayModeText() {
    return `Mutation ${replayer.applyMutationEvents ? "ON" : "OFF"}, Scripts ${replayer.executePageScripts ? "ON" : "OFF"}`;
  }
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="sr-row">
        <label style="display:flex;align-items:center;gap:6px;">
          Snapshot ms
          <input data-id="snapshot-interval" type="number" min="500" step="100" value="3000" style="width:110px;margin:0;" />
        </label>
      </div>
      <div class="sr-row">
    ...
    return {
      ...
      snapshotIntervalInput: panel.querySelector("[data-id='snapshot-interval']"),
  ...
  class SessionRecorder {
    constructor(options = {}) {
      ...
      this.eventsToRecord = options.events || [...BUTTON_ACTION_EVENTS, "mousemove", "input", "change", "submit", "scroll"];
      this.shouldMaskInputValue = options.maskInputValue || false;
      this.mousemoveSampleMs = options.mousemoveSampleMs || 20;
      this.snapshotIntervalMs = this.normalizeSnapshotInterval(options.snapshotIntervalMs);
      ...
      this.delayedSnapshotTimerId = null;
      this.lastSnapshotHtml = "";
    }

    normalizeSnapshotInterval(value) {
    ...
    setSnapshotIntervalMs(value) {
      this.snapshotIntervalMs = this.normalizeSnapshotInterval(value);
      if (this.isRecording) {
    ...
    scheduleDelayedSnapshot(reason, delayMs = 260) {
      if (!this.isRecording) {
        return;
      }

      if (this.delayedSnapshotTimerId) {
        clearTimeout(this.delayedSnapshotTimerId);
        this.delayedSnapshotTimerId = null;
      }

      this.delayedSnapshotTimerId = setTimeout(() => {
        this.captureSnapshot(reason);
        this.delayedSnapshotTimerId = null;
      }, Math.max(0, Number(delayMs) || 0));
    }

    captureSnapshot(reason = "manual", force = false) {
      if (!this.isRecording) {
        return false;
      }

      const html = getSnapshotHtmlForRecording();
      if (!force && html === this.lastSnapshotHtml) {
        return false;
      }
    ...
    stop() {
      ...
      this.captureSnapshot("final", true);
    ...
    attachMutationObserver() {
      this.mutationObserver = new MutationObserver((mutationRecords) => {
        mutationRecords.forEach((mutation) => {
          ...
          const targetTag = mutation.target instanceof Element ? mutation.target.tagName.toLowerCase() : "";
          const isHeadMutation = targetTag === "head" || targetTag === "style" || targetTag === "link";
          const isStyleAttrMutation =
            mutation.type === "attributes" &&
            mutation.attributeName &&
            ["class", "style", "href", "media"].includes(String(mutation.attributeName).toLowerCase());

          if (isHeadMutation || isStyleAttrMutation) {
            this.scheduleDelayedSnapshot("style_or_head_update", 220);
          }
        });
      });
    ...
    attachEventListeners() {
      this.eventsToRecord.forEach((eventName) => {
        const handler = (event) => {
          ...
          if (this.shouldIgnoreNode(targetEl || event.target)) {
            return;
          }
          ...
          if (isButtonActionEvent(eventName)) {
            if (!shouldRecordButtonAction(eventName, event)) {
              return;
            }

            this.record("event", {
              ...common,
              ...extractButtonActionData(event)
            });

            if (eventName === "click") {
              const anchor = targetEl instanceof Element ? targetEl.closest("a[href]") : null;
            ...
            if (shouldCaptureAfterButtonAction(eventName, event)) {
              this.scheduleDelayedSnapshot(`post_${eventName}`, 320);
            }
            return;
          }
          ...
          if (eventName === "input" || eventName === "change") {
            ...
            this.scheduleDelayedSnapshot(`post_${eventName}`, 260);
            return;
          }
          ...
          if (eventName === "scroll") {
            ...
            this.scheduleDelayedSnapshot("post_scroll", 320);
            return;
          }
          ...
          if (eventName === "submit") {
            ...
            this.scheduleDelayedSnapshot("post_submit", 380);
            return;
          }
        ...
        target.addEventListener(eventName, handler, {
          capture: true,
          passive: eventName === "scroll" || eventName === "mousemove" || eventName === "touchstart"
        });
    ...
    attachNavigationListeners() {
      this.navigationEventsToRecord.forEach((eventName) => {
        ...
        const handler = (event) => {
          ...
          this.scheduleDelayedSnapshot(`navigation_${eventName}`, 280);
        };
    ...
    patchHistoryMethods() {
      ...
      window.history.pushState = function patchedPushState(state, title, url) {
        ...
        recorder.scheduleDelayedSnapshot("navigation_history_pushstate", 260);
        return result;
      };
      ...
      window.history.replaceState = function patchedReplaceState(state, title, url) {
        ...
        recorder.scheduleDelayedSnapshot("navigation_history_replacestate", 260);
        return result;
      };
  ...
  class SessionReplayer {
    ...
    play(options = {}) {
      ...
      const snapshots = this.payload.events.filter((event) => event.type === "snapshot" && event.data && event.data.html);
      const replayEvents = this.payload.events.filter((event) => {
        return event.type === "event" || event.type === "mutation";
      });

      const baseSnapshot = this.selectBaseSnapshot(snapshots, replayEvents);
      if (!baseSnapshot) {
        throw new Error("snapshot event is missing");
      }
      this.setViewport(baseSnapshot.data && baseSnapshot.data.viewport);
      const timelineEvents = this.buildTimelineEvents(replayEvents, baseSnapshot);

      if (!timelineEvents.length) {
        this.renderSnapshot(
          baseSnapshot.data.html,
          baseSnapshot.data.url,
          baseSnapshot.data.styleSummary,
          baseSnapshot.data.iframeSummary,
          () => {
          this.onStatus("Snapshot rendered. No replayable events after selected base snapshot.");
          this.isPlaying = false;
          }
        );
        return;
      ...
      this.renderSnapshot(
        ...
        baseSnapshot.data.iframeSummary,
        () => {
        const mutationMode = this.applyMutationEvents ? "ON" : "OFF";
        this.onStatus(
          `Replay started. speed=${this.speed}x, baseSnapshot=${baseSnapshot.data.reason || "snapshot"}@${round(Number(baseSnapshot.timeOffsetMs) || 0)}ms, timelineEvents=${timelineEvents.length}, mutationApply=${mutationMode}`
        );
        this.runTimeline(timelineEvents);
        }
      );
    }

    selectBaseSnapshot(snapshots, replayEvents) {
      if (!snapshots.length) {
        return null;
      }

      if (!replayEvents.length) {
        return this.selectBestVisualSnapshot(snapshots);
      }

      const eventOnly = replayEvents.filter((event) => event.type === "event");
      const firstMeaningfulEvent = eventOnly.find((event) => {
        const eventType = event && event.data && event.data.eventType;
        return eventType &&
          !["mousemove", "visibilitychange", "pageshow", "pagehide"].includes(String(eventType).toLowerCase());
      });

      if (!firstMeaningfulEvent) {
        return this.selectBestVisualSnapshot(snapshots);
      }

      const pivotTime = Number(firstMeaningfulEvent.timeOffsetMs) || 0;

      let selected = snapshots[0];
      snapshots.forEach((snapshot) => {
        const ts = Number(snapshot.timeOffsetMs) || 0;
        if (ts <= pivotTime) {
          selected = snapshot;
        }
      });
    ...
    buildTimelineEvents(replayEvents, baseSnapshot) {
      const baseTime = Number(baseSnapshot && baseSnapshot.timeOffsetMs) || 0;
      return replayEvents.filter((event) => (Number(event.timeOffsetMs) || 0) >= baseTime);
    }

    selectBestVisualSnapshot(snapshots) {
      return snapshots.reduce((best, current) => {
        if (!best) {
          return current;
        }

        const bestStyles = Number(best.data && best.data.styleSummary && best.data.styleSummary.stylesheetCount) || 0;
        const currStyles = Number(current.data && current.data.styleSummary && current.data.styleSummary.stylesheetCount) || 0;
        if (currStyles !== bestStyles) {
          return currStyles > bestStyles ? current : best;
        }

        const bestHtml = String(best.data && best.data.html || "").length;
        const currHtml = String(current.data && current.data.html || "").length;
        if (currHtml !== bestHtml) {
          return currHtml > bestHtml ? current : best;
        }

        const bestTs = Number(best.timeOffsetMs) || 0;
        const currTs = Number(current.timeOffsetMs) || 0;
    ...
    runTimeline(events) {
      const step = () => {
        ...
        const timelineType = getTimelineEventType(current);
        if (timelineType) {
          const skipLabel = current.type === "mutation" && !this.applyMutationEvents ? " (apply skipped)" : "";
          this.onStatus(`Timeline ${this.currentIndex + 1}/${events.length}: ${timelineType}${skipLabel}`);
        }
        const continueStep = () => {
          this.currentIndex += 1;
    ...
    renderSnapshot(rawHtml, baseUrl, expectedStyleSummary, expectedIframeSummary, onReady) {
      if (!this.iframe) {
      ...
      this.iframe.onload = () => {
        ...
        if (doc) {
          restoreIframeSources(doc, expectedIframeSummary);
        }

        this.waitForDocumentVisualReady(expectedStyleSummary, expectedIframeSummary).finally(() => {
          this.updateViewportScale();
    ...
    waitForDocumentVisualReady(expectedStyleSummary, expectedIframeSummary, maxWaitMs = 1800) {
      const doc = this.iframe && this.iframe.contentDocument;
      if (!doc) {
      ...
      const expectedStylesheetCount = Math.max(
        Number(expectedStyleSummary && expectedStyleSummary.stylesheetCount) || 0,
        Number(expectedStyleSummary && expectedStyleSummary.linkCount) || 0
      );

      const stylesheetCountPromise = this.waitForStylesheetCount(doc, expectedStylesheetCount, maxWaitMs);
      const iframeReadyPromise = this.waitForIframeLoads(doc, expectedIframeSummary, maxWaitMs);

      const fontPromise = doc.fonts && doc.fonts.ready
        ? Promise.race([doc.fonts.ready.catch(() => {}), sleep(maxWaitMs)])
        : Promise.resolve();

      return Promise.race([
        Promise.all([...linkPromises, fontPromise, stylesheetCountPromise, iframeReadyPromise]).then(() => this.waitForAnimationFrames(2)),
        sleep(maxWaitMs)
      ]);
    }

    waitForIframeLoads(doc, expectedIframeSummary, maxWaitMs) {
      const frames = Array.from(doc.querySelectorAll("iframe"));
      if (!frames.length) {
        return Promise.resolve();
      }

      const expectedSrcSet = new Set(
        Array.isArray(expectedIframeSummary)
          ? expectedIframeSummary
              .map((item) => normalizeRecordedIframeSrc(item && (item.currentSrc || item.src)))
              .filter(Boolean)
          : []
      );

      const candidates = frames.filter((frame) => {
        const src = normalizeRecordedIframeSrc(frame.getAttribute("src") || frame.src);
        if (!src) {
          return false;
        }
        if (!expectedSrcSet.size) {
          return true;
        }
        return expectedSrcSet.has(src);
      });
      ...
      return Promise.race([
        Promise.all(candidates.map((frame) => new Promise((resolve) => {
          let done = false;
          const finish = () => {
            if (done) {
              return;
          ...
          setTimeout(finish, Math.min(1200, maxWaitMs));
        }))),
        sleep(maxWaitMs)
      ]);
    }

    waitForStylesheetCount(doc, expectedCount, maxWaitMs) {
      ...
      return new Promise((resolve) => {
        ...
        const tick = () => {
          ...
          if (doc.styleSheets.length >= expectedCount) {
            resolve();
            return;
          }
    ...
    waitForAnimationFrames(count = 1) {
      ...
      return new Promise((resolve) => {
        const next = () => {
          ...
          this.waitForAnimationFrames(count - 1).then(resolve);
        };
  ...
  function shouldApplyChildListMutation(target, data) {
    ...
    // Full-document level replacements are very fragile and frequently break replay layout.
    ...
    const isClearMutation = addedCount === 0 && removedCount > 0 && html.trim() === "";
    if (isClearMutation) {
      return false;
    }
    ...
    const largeIncomingHtml = html.length > 3500;
    const largeCurrentHtml = String(target.innerHTML || "").length > 3500;
    const largeSubtree = target.childElementCount > 20;

    // Paths based only on nth-of-type become unstable after many DOM changes.
    if (!idBasedPath && nthBasedPath && (largeIncomingHtml || largeCurrentHtml || largeSubtree)) {
      return false;
    }
  ...
  function getTimelineEventType(event) {
    ...
    if (event.type === "event") {
      return String(event.data && event.data.eventType || "event");
    }

    if (event.type === "mutation") {
      return String(event.data && (event.data.eventType || event.data.mutationType) || "mutation");
    }
  ...
  function getIframeSummary(shouldIgnoreNode = () => false) {
    ...
    return frames
      ...
      .map((frame) => ({
        ...
        currentSrc: normalizeRecordedIframeSrc(frame.currentSrc || frame.src || frame.getAttribute("src")),
        width: frame.getAttribute("width") || null,
        height: frame.getAttribute("height") || null
      }));
  }

  function restoreIframeSources(doc, iframeSummary) {
  ...
  function normalizeRecordedIframeSrc(value) {
    ...
    if (!src || /^about:blank$/i.test(src) || /^javascript:/i.test(src)) {
      return null;
    }
  ...
  function isButtonActionEvent(eventName) {
    return BUTTON_ACTION_EVENTS.includes(eventName);
  }
  ...
  function isButtonLikeElement(element) {
    ...
    return Boolean(
      element.closest(
        "button, [role='button'], input[type='button'], input[type='submit'], input[type='reset'], a[role='button'], summary, [aria-pressed]"
      )
    );
  }

  function shouldRecordButtonAction(eventName, event) {
    const targetEl = getEventElement(event);
    if (!isButtonLikeElement(targetEl)) {
      return false;
    }
  ...
  function shouldCaptureAfterButtonAction(eventName, event) {
    if (["click", "dblclick", "pointerup", "mouseup", "touchend", "contextmenu", "auxclick"].includes(eventName)) {
      return true;
    }
  ...
  function extractButtonActionData(event) {
    const targetEl = getEventElement(event);
    const data = {
      buttonAction: true,
      targetTag: targetEl ? targetEl.tagName.toLowerCase() : null
    };
    ...
    if (typeof event.clientX === "number" && typeof event.clientY === "number") {
      ...
      if (pointerMeta) {
        Object.assign(data, pointerMeta);
      }
    }
    ...
    if (event.changedTouches && event.changedTouches.length) {
      ...
      data.viewportWidth = window.innerWidth;
      data.viewportHeight = window.innerHeight;
      data.touchCount = event.touches ? event.touches.length : undefined;
    }
    ...
    if (typeof event.key === "string") {
      ...
      data.repeat = Boolean(event.repeat);
    }
